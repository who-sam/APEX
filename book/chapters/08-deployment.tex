...
\begin{table}[H]
  ...
  \begin{tabularx}{\linewidth}{l l X}
    ...
    \midrule
    \texttt{DB\_HOST}             & \texttt{localhost}        & Postgres host \\
    \texttt{DB\_PORT}             & \texttt{5432}             & Postgres port \\
    \texttt{DB\_USER}             & \texttt{postgres}         & Postgres user \\
    \texttt{DB\_PASSWORD}         & \texttt{postgres}         & Postgres password \\
    \texttt{DB\_NAME}             & \texttt{apex}             & Postgres database \\
    \texttt{JWT\_SECRET}          & \emph{required, $\geq$32 chars} & HMAC secret for JWT signing \\
    \texttt{JUDGE0\_URL}          & \texttt{https://ce.judge0.com} & Judge0 base URL \\
    \texttt{APP\_URL}             & \texttt{http://localhost:5173} & used in password-reset email links \\
    \texttt{GOOGLE\_CLIENT\_ID}   & \emph{empty disables OAuth} & Google OAuth audience \\
    \texttt{SMTP\_HOST}, \texttt{SMTP\_PORT}, \texttt{SMTP\_USER}, \texttt{SMTP\_PASSWORD}, \texttt{SMTP\_FROM} & \emph{empty disables email} & SMTP relay credentials \\
    \bottomrule
  \end{tabularx}
\end{table}
